using System;
using System.Collections.Generic;
using System.Text;

namespace MyAnimeWorld.Common.Utilities.Constants
{
    public static class NumericConstants
    {
        public const int Number_Of_Animes_Per_Page = 12;

        public const int Number_Of_Users_Per_Page = 30;

        public const int Minimum_Search_Term_Length = 3;

        public const int Default_Animes_Number_For_Category = 4;
        
        public const int Index_Pagination_Number_Of_Pages = 5;

        public const int Minimum_Links_For_Episode = 1;

        //Changing these properties will require a change to their error statement as well!
        public const int Minimum_Comment_Length = 5;

        public const int Minimum_Title_Length = 3;

        public const int Maximum_Title_Length = 35;

        public const int Minimum_Description_Length = 5;
    }
}
